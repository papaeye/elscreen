...
(defcustom elscreen-prefix-key (kbd "C-z")
...
(defvar elscreen-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c") 'elscreen-create)
    (define-key map (kbd "c")   'elscreen-create)
    (define-key map (kbd "C")   'elscreen-clone)
    (define-key map (kbd "C-k") 'elscreen-kill)
    (define-key map (kbd "k")   'elscreen-kill)
    (define-key map (kbd "M-k") 'elscreen-kill-screen-and-buffers)
    (define-key map (kbd "K")   'elscreen-kill-others)
    (define-key map (kbd "C-p") 'elscreen-previous)
    (define-key map (kbd "p")   'elscreen-previous)
    (define-key map (kbd "C-n") 'elscreen-next)
    (define-key map (kbd "n")   'elscreen-next)
    (define-key map (kbd "C-a") 'elscreen-toggle)
    (define-key map (kbd "a")   'elscreen-toggle)
    (define-key map (kbd "'")   'elscreen-goto)
    (define-key map (kbd "\"")  'elscreen-select-and-goto)
    (define-key map (kbd "0")   'elscreen-jump-0)
    (define-key map (kbd "1")   'elscreen-jump)
    (define-key map (kbd "2")   'elscreen-jump)
    (define-key map (kbd "3")   'elscreen-jump)
    (define-key map (kbd "4")   'elscreen-jump)
    (define-key map (kbd "5")   'elscreen-jump)
    (define-key map (kbd "6")   'elscreen-jump)
    (define-key map (kbd "7")   'elscreen-jump)
    (define-key map (kbd "8")   'elscreen-jump)
    (define-key map (kbd "9")   'elscreen-jump-9)
    (define-key map (kbd "C-s") 'elscreen-swap)
    (define-key map (kbd "C-w") 'elscreen-display-screen-name-list)
    (define-key map (kbd "w")   'elscreen-display-screen-name-list)
    (define-key map (kbd "C-m") 'elscreen-display-last-message)
    (define-key map (kbd "m")   'elscreen-display-last-message)
    (define-key map (kbd "C-t") 'elscreen-display-time)
    (define-key map (kbd "t")   'elscreen-display-time)
    (define-key map (kbd "A")   'elscreen-screen-nickname)
    (define-key map (kbd "b")   'elscreen-find-and-goto-by-buffer)
    (define-key map (kbd "C-f") 'elscreen-find-file)
    (define-key map (kbd "C-r") 'elscreen-find-file-read-only)
    (define-key map (kbd "d")   'elscreen-dired)
    (define-key map (kbd "M-x") 'elscreen-execute-extended-command)
    (define-key map (kbd "i")   'elscreen-toggle-display-screen-number)
    (define-key map (kbd "T")   'elscreen-toggle-display-tab)
    (define-key map (kbd "?")   'elscreen-help)
    (define-key map (kbd "v")   'elscreen-display-version)
    (define-key map (kbd "j")   'elscreen-link)
    (define-key map (kbd "s")   'elscreen-split)
    map)
  "Keymap for ElScreen.")
...
(defsubst elscreen-get-frame-confs (frame)
  (cdr (assq frame elscreen-frame-confs)))
...
(defsubst elscreen-set-conf-list (type conf-list)
  (let* ((frame-conf (elscreen-get-frame-confs (selected-frame)))
         (pair (assq type frame-conf)))
    (when pair
      (setcdr pair conf-list))))

(defun elscreen-get-screen-property (screen)
  (let ((screen-property-list (elscreen-get-conf-list 'screen-property)))
    (cdr (assq screen screen-property-list))))

(defun elscreen-set-screen-property (screen property)
  (let ((screen-property-list (elscreen-get-conf-list 'screen-property)))
    (setq screen-property-list
          (elscreen--put-alist screen property screen-property-list))
...
from `elscreen-frame-confs', a cons cell."
  (let ((screen-property (elscreen-get-screen-property screen)))
    (cdr (assq 'window-configuration screen-property))))

(defun elscreen-set-window-configuration (screen winconf)
  "Set pair of window-configuration and marker of SCREEN to WINCONF."
  (let ((screen-property (elscreen-get-screen-property screen)))
    (setq screen-property
          (elscreen--put-alist 'window-configuration winconf screen-property))
...
(defun elscreen-get-screen-nickname (screen)
  ...
  (let ((screen-property (elscreen-get-screen-property screen)))
    (cdr (assq 'nickname screen-property))))

(defun elscreen-set-screen-nickname (screen nickname)
  "Set nickname of SCREEN to NICKNAME."
  (let ((screen-property (elscreen-get-screen-property screen)))
    (setq screen-property
          (elscreen--put-alist 'nickname nickname screen-property))
    (elscreen-set-screen-property screen screen-property)))
       ...
       (elscreen-save-screen-excursion
        (mapc
         (lambda (screen)
           ...
           (setq screen-to-name-alist
                 (elscreen--put-alist screen screen-name screen-to-name-alist)))
         screen-list))
...
(defun elscreen-select-and-goto ()
  ...
  (let* ((screen-list (sort (elscreen-get-screen-list) '<))
                      ...
                      (lambda (screen)
                        (format "  %d%s %s\n" screen
                                ...
                                (elscreen-truncate-screen-name
                                 (cdr (assq screen screen-to-name-alist))
                                 truncate-length)))
                      screen-list nil)))
    ...
    (cond
     ...
     ((cdr (assoc command-or-target-screen command-list))
      (funcall (cdr (assoc command-or-target-screen command-list))))
     (t
             ...
             (lambda (screen)
               (list (string-to-char (number-to-string screen))
                     ...
                     (format "%d%s %s" screen
                             (elscreen-status-label screen)
                             (elscreen-truncate-screen-name
                              (cdr (assq screen screen-to-name-alist)) 25))
                     'elscreen-jump
                     :keys (format "%s %d"
...
(defsubst elscreen-tab-create-keymap (&rest definitions)
  (let ((keymap (make-sparse-keymap))
    ...
    (while definitions
      (setq key-function-pairs
            (elscreen--put-alist (car definitions) (cadr definitions)
                                 key-function-pairs))
                       ...
                       (propertize
                        (concat
                         ...
                         (propertize
                          ...
                          'help-echo (cdr (assq screen screen-to-name-alist))
                          'local-map (elscreen-tab-create-keymap
                                      'mouse-1 `(lambda (e)
                                                  (interactive "e")
                                                  (elscreen-goto ,screen))))
